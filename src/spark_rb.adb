...
is
   ...
   is (C <= Natural'Last - C and then 2 * C - 1 <= Natural'Last - 2 * C);
   ...
   begin
      ...
      for I in 1 .. Length (OldB) loop
         ...
         pragma
           Assert
             (Mask (B, B.Read + (I - 1)) /= Mask (B, B.Read + Length (OldB)));
         ...
         pragma
           Loop_Invariant
             ((for all K in 1 .. I => Get (OldB, K) = Get (B, K)));
      end loop;
   end Push;
   ...
   begin
      ...
      for I in 1 .. Length (B) loop
         ...
         pragma
           Loop_Invariant
             ((for all K in 1 .. I => Get (B, K) = Get (OldB, K + 1)));
      end loop;
   end Pop;
   ...
   begin
      if To_Length < Length (B) then
         ...
         for I in 1 .. Length (B) loop
            Lemma_Pop_Shifts_Back_Elements
              (R, Rp, C, To_Big_Integer (I) - 1, N - Np);
            pragma
              Assert
                (Mask (B, OldB.Read + ((I - 1) + (Length (OldB) - Length (B))))
                   = Mask (B, B.Read + (I - 1)));
            pragma
              Assert
                (Get (OldB, I + (Length (OldB) - Length (B))) = Get (B, I));
            pragma
              Loop_Invariant
                (for all K in 1 .. I =>
                   Get (OldB, K + (Length (OldB) - Length (B))) = Get (B, K));
         end loop;
      end if;
   end Truncate_Back;
