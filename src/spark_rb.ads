...
generic
   ...
   Uninit : Element;
package Spark_Rb with SPARK_Mode => On is
   ...
   with
     Post =>
       Capacity_Is_Not_Too_Large'Result
       = (C <= Natural'Last - C and then 2 * C - 1 <= Natural'Last - 2 * C);

   function Is_Valid (B : Buffer) return Boolean
   with
     Pre  => Capacity_Is_Not_Too_Large (B.Capacity),
     Post =>
       Is_Valid'Result
       = (B.Read < 2 * B.Capacity
   ...
   with
     Dynamic_Predicate =>
       Capacity_Is_Not_Too_Large (Valid_Buffer.Capacity)
       and then Is_Valid (Valid_Buffer);

   function Buffer_Init (Capacity : Capacity_Type) return Valid_Buffer
   with
     Pre  => Capacity_Is_Not_Too_Large (Capacity),
     Post =>
       Buffer_Init'Result.Capacity = Capacity
       and then Is_Empty (Buffer_Init'Result);

   function Mask (B : Valid_Buffer; I : Natural) return Positive
   with
     Post =>
       Mask'Result <= B.Capacity and then Mask'Result = 1 + I mod B.Capacity;
   ...
   with
     Pre  => Capacity_Is_Not_Too_Large (B.Capacity),
     Post =>
       Length'Result < 2 * B.Capacity
       and then To_Big_Integer (Length'Result)
                = (To_Big_Integer (B.Write) - To_Big_Integer (B.Read))
                  mod To_Big_Integer (2 * B.Capacity);

   function Is_Empty (B : Valid_Buffer) return Boolean
   with
     Post =>
       Is_Empty'Result = (B.Read = B.Write)
   ...
   function Get (B : Valid_Buffer; I : Positive) return Element
   with
     Pre  => I <= Length (B),
     Post => Get'Result = B.Memory (Mask (B, B.Read + (I - 1)));

   procedure Push (B : in out Valid_Buffer; V : Element)
   with
     Pre  => not Is_Full (B),
     Post =>
       Length (B'Old) + 1 = Length (B)
       and then (for all I in 1 .. Length (B'Old) =>
                   Get (B'Old, I) = Get (B, I))
       and then Get (B, Length (B)) = V;

   procedure Pop (B : in out Valid_Buffer; V : out Element)
   with
     Pre  => not Is_Empty (B),
     Post =>
       Length (B) + 1 = Length (B'Old)
       and then (for all I in 1 .. Length (B) =>
                   Get (B, I) = Get (B'Old, I + 1))
       and then Get (B'Old, 1) = V;
   ...
   with
     Post =>
       Length (B) = Natural'Min (To_Length, Length (B'Old))
       and then (for all I in 1 .. Length (B) =>
