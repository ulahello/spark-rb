...
is
   ...
   with
     Post               =>
       ...
       = (case N is
            when 0      => False,
            when 1      => True,
            when others => N mod 2 = 0 and then Is_Power_Of_Two (N / 2)),
   ...
   with
     ...
     Subprogram_Variant => (Decreases => K, Decreases => N);
   ...
   with Ghost, Pre => M /= 0, Post => (-N) mod (-M) = -(N mod M);
   ...
   with Ghost, Pre => M /= 0 and then A = B + K * M, Post => K = (A - B) / M;

   procedure Lemma_Mod_Idempotent (N, M : Big_Integer)
   with Ghost, Pre => M /= 0, Post => (N mod M) = (N mod M) mod M;

   procedure Lemma_Mod_Nop (N, M : Big_Integer)
   with Ghost, Pre => 0 <= N and then N < M, Post => (N mod M) = N;

   procedure Lemma_Mod_Add_Simp (A, B, M : Big_Integer)
   with Ghost, Pre => M /= 0, Post => (A + B mod M) mod M = (A + B) mod M;
   ...
   with
     Ghost,
     Pre  => M /= 0,
     Post => (if A mod M = B mod M then (A + K) mod M = (B + K) mod M);
   ...
   with
     Ghost,
     Pre  => M /= 0 and then N /= 0 and then A mod (M * N) = B mod (M * N),
     Post => (A mod M = B mod M) and then (A mod N = B mod N);
   ...
   with
     Ghost,
     Pre  => (0 <= R and then R < 2 * C) and then Rp = (R + dR) mod (2 * C),
